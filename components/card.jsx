...
const Card = ({ anime }) => {
  return ( 
    <Link className={styles.card} href={`/anime/${anime.id}/?menu=info`} as={`/anime/${anime.id}/?menu=info`}>
      <Image className={styles.poster} src={`${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/Posters/${anime.poster}`} alt={anime.title} width={150} height={215} loading = 'lazy'  />
