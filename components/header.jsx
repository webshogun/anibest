        ...
        <div className={styles.wrapper}>
                ...
                <div className={styles.results}>
                  {searchResults.map((result) => (
                    <Link className={styles.result} key={result.id} href={`/anime/${result.id}/?menu=info`} onClick={handleSearchResultClick}>
                      <Image className={styles.poster} src={`${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/Posters/${result.poster}`} alt={result.title} width={75} height={105} /> 
          ...
          {user ? (
            <Link className={styles.account} href='/profile'>
              {avatar ? (
                <Image className={styles.avatar} src={`${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/avatars/${avatar}`} width={30} height={30} loading = 'lazy'  />
              ) : (
